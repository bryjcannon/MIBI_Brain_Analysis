...
allDataStitch = uint8(allDataStitch);
...
%         end
        ...
        if pointList(currPoint) <= numel(currRun) + pointsProcessed % number of point inthe botom run.
            ...
            currData = imread([TIFs_PATH{runNumber}, '/Point', num2str(pointList(currPoint) - pointsProcessed),'/', channel]);
...
% colormap(gray);
% colorbar;
% title(p{1}.massDS.Label{plotChannelInd});
%set(gca,'xtick',[],'ytick',[]);
...
imwrite(data,[OutputFolder,'/StitchSegGated_',channel]);
