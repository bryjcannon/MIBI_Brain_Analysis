...
allDataStitch = zeros((xNumPoint + 2) * dataSize, (yNumPoint + 2) * dataSize, img_color); %BRYAN ADDITION: change 3 to 1 if working with binary or grayscale images
...
for i=1:xNumPoint
    ...
    for j=1:yNumPoint
        ...
        currData = imread([TIFs_PATH, '/Point', num2str(pointList(currPoint)), '/TIFs/', channel]);
        
        currData = currData(1 : dataSize, 1 : dataSize, :); % BRYAN CHANGED - got rid of original 3:datasize + 2 to accomadate all objects (no removal of borders)

        % Get first position
        if (i == 1) && (j == 1) % first point. No coregistering
            currPos = startPosGlobal;
            %currData = insertText(currData,[20 40],['Point ',num2str(currPoint)],'FontSize',80,'TextColor','red','BoxColor','black','BoxOpacity',0); 
...
%Make and save images and close figures
%imwrite((uint16(data)),[OutputFolder,'/Tiled_',channel]);
%capImage=max(max(max(data)));
%data(data>capImage) = capImage;
% imagesc(data);
% colormap(gray);
%saveas(gcf,'/Volumes/KosaliaMIBI/MIBIData Matlab/May2018/180502_Tile5RunSN/180502_SN_Panel3ug_Final/tile.fig');
