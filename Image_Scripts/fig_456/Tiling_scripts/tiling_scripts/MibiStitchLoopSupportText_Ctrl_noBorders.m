...
for i=1:xNumPoint
    ...
    for j=1:yNumPoint
        ...
        %end
        currData = currData(1 : dataSize, 1 : dataSize, :) ; % BRYAN CHANGED - got rid of original (3 : dataSize + 2, 3 : dataSize + 2,:) to accomodate all objects (no removal of borders) (also in line 203, 300)
        %currData=img(:,:,[1 1 1]);
...
pointList = 1:105; % BRYAN changed from: pointsProcessed + 1:pointsProcessed + 105
...
%         end
        ...
        currRun = dir([TIFs_PATH{runNumber}, '/Point*']);
         ...
         %else % the consecutive run
             ...
             %currData = double(imread([TIFs_PATH{runNumber}, '/Point', num2str(pointList(currPoint) - pointsProcessed), '/TIFs/', channel]));
         %end
        currData = currData(1 : dataSize, 1 : dataSize, :);
        %currData=img(:,:,[1 1 1]);
...
%for Bryan's pt numbers
pointList = 1:70; % BRYAN changed from: 206:275
