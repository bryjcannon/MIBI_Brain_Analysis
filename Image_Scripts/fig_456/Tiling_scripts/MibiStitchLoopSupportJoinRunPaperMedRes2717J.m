...
allDataStitch = zeros((xNumPoint + 5) * dataSize, (yNumPoint + 3) * dataSize);
...
ydRight = 20; % Shift this many pixels when moving right 
...
xdTop = -50; % Should be negative or 0. Controls vertical coregistration when moving up one row. Positive value would yield blank space between rows
...
for i=1:xNumPoint
    if currPoint > 196 %number of points in first run 
            break;
    end
    xloc = xNumPoint - i + 1;
    for j=1:yNumPoint
...
% runNumber = runNumber + 1;
% currPoint = 0;
% ydRight = 0; % Shift this many pixels when moving right. horizontal
% xdRight = 0; % Vertical tilt of the image, shift this many pixels up each time when moving right
% ydTop = 0; % Shift right by ydTop pixels when moving up one row. Horizontal tilt
...
% ydTop_run = 0; % Shift right by ydTop pixels when moving up one row. Horizontal tilt
% xdTop_run = 0; % Should be negative or 0. Controls vertical coregistration when moving up one row. Positive value would yield blank space between rows
% 
% for i=1:xNumPoint
%     if currPoint > 45 %number of points in second run 
%             break;
%     end
%     xloc = xNumPoint - i + 1;
%     for j=1:yNumPoint
...
% %         % Get first position
%         if (i == 1) && (j == 1) % first point. No coregistering
%             currPos = ([(xloc - 1/2 + 14) * dataSize, (1/2) * dataSize]);
...
imwrite((uint16(data)),[OutputFolder,'/StitchedNoFUNTOTALTAU_',channel]);
