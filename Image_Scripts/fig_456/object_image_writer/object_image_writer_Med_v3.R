      ...
      for (obj_index in 1:length(object_types)) {
        ...
        for (uniq_object in unique(pointdata$obj_id)){
          ...
          if (objects_clustered == T & grayscale == F) {
            obj_mask_r[obj_mask == uniq_object] = col2rgb(palette)[1, subset(pointdata, obj_id == uniq_object)$FlowSOM_ids]
            obj_mask_g[obj_mask == uniq_object] = col2rgb(palette)[2, subset(pointdata, obj_id == uniq_object)$FlowSOM_ids]
            obj_mask_b[obj_mask == uniq_object] = col2rgb(palette)[3, subset(pointdata, obj_id == uniq_object)$FlowSOM_ids]
          }
