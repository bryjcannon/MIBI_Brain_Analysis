...
##### FILE & DATA SETTINGS - user must adjust these! #####
  ...
  data_for_images <- master_data_all_cuberoot
  ...
  for (i in 1:length(run_names)) {
    ...
    for (p in point_list[[i]]) {
      ...
      for (obj_index in 1:length(object_types)) {  
        ...
        pointdata <- subset(data_for_images, run_type_id == region & point_id == p & obj_type_id == object_types[obj_index] & cluster_id != 10)
        for (uniq_object in unique(pointdata$obj_id)){
          ...
          else if ((objects_clustered == F) & (objects_for_annotation == F)) {
