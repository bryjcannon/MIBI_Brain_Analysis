...
##### FILE & DATA SETTINGS - user must adjust these! #####
  ...
  save_name <- 'DGtoCA1_nu_colors'
  # shorthand for regions or runs, used in run_type_id
  run_names <- c('MedAD')
  # rgb or grayscale
  grayscale <- F
  # object types you plan to cluster)
  object_types <- c('cells', 'amyloid-pathies', 'tau-pathies') #'cells', 'amyloid-pathies', 'tau-pathies'
  # boolean denoting if clustering was performed on the objects (T) or not (F)
  objects_clustered <- F
  clustering_variable <- 'FlowSOM_ids'
  include_clusters <- c(1, 2, 3, 4, 5, 6) #represents c('tangles','plaques','microglia','endothelial','neurons','non-immune glia') OR de_novo_regions 1-5
  ...
  spatial_id_assign <- F
  # color palete of choice, use second line to see color choices
  palette <- color2
