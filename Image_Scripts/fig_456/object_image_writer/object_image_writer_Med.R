...
##### FILE & DATA SETTINGS - user must adjust these! #####
  ...
  save_name <- 'microglia_region_masks'
  ...
  objects_clustered <- T
  ignore_clusters <- c(3)
  # boolean denoting if region labeling was performed on the objects (T) or not (F)
  regions_clustered <- F
  ignore_regions <- c(3)
    ...
    for (p in point_list[[i]]) {
      ...
      for (obj_index in 1:length(object_types)) {  
        ...
        for (uniq_object in unique(pointdata$obj_id)){
          ...
          if (regions_clustered == T) {pointdata <- filter(pointdata, !region_cluster_id %in% ignore_regions)}
          ...
          # produce grayscale masks of cells/objects by region_cluster
          else if (regions_clustered == T) {
            obj_mask_r[obj_mask == uniq_object] = subset(pointdata, obj_id == uniq_object)$region_cluster_id
          }
          # color objects based on unique RGB_ids calculated earlier
          else if (objects_for_tiling == T) {
          ...
          else if ((objects_clustered == F) & (objects_for_tiling == F)) {
        ...
        else if (objects_for_tiling == T) {type_of_image <- 'unique_ids'}
        else if (regions_clustered == T) {type_of_image <- 'region_ids'}
