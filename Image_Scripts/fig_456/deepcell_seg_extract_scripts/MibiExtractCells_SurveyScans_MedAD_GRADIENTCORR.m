...
datasize=512;
 ...
 path = '/Volumes/BryJC_Stanford/Data/Cleaned_Data_Kausi/MedRes_MedADCase/NoAuBGFFtDenoised_New/';
...
pathSegment = [path, 'segmentationDC/single_cell_dynamic_expansion_GRADIENTCORR'];
resultsDir = [path,'segmentationDC/fcs_cells_GRADIENTCORR'];
mkdir(resultsDir);
...
for p = 1:NumPoints
    ...
    pointNumber=p-1; % this dataset starts at Point 0, goes to Point 259 (260 pts total)
    
    % load data and get nuclear markers
    if isfile([path,'corrected/Point',num2str(pointNumber),'/dataNoFFTNoise.mat'])
       load([path,'corrected/Point',num2str(pointNumber),'/dataNoFFTNoise.mat']);
       channelnames = massDS.Label;
       [~, clusterChannelsInds] = ismember(clusterChannels,massDS.Label);
       channelNum = length(massDS);
    else       
        tifsFolder= dir(fullfile([path,'corrected/Point',num2str(pointNumber),'/TIFs'],'*.tif'));
