#### Region Analysis ####
  ...
  glia_struct2 <- c('Iba1', 'CD45', 'ApoE4', 'CD33Lyo')
  ...
  other_struct <- c('Amyloidbeta140','Amyloidbeta142','PHF1Tau','CD33Lyo','X8OHGuano','CD47','Calretinin','Calbindin','Parvalbumin','PanGAD6567','VGAT','TH','SERT','VGLUT1','VGLUT2','PSD95','Synaptophysin','MAP2','MFN2')
...
#Heatmaps
    ...
    #organize naming, ordering schema  
      ...
      #heat[heat>1] <- 1
      heatmaply(heatmaply::normalize(heat_region[c(1:6),panel[-c(1:4,44:45)]]), grid_gap = 1, Rowv = FALSE, labRow = rownames(heat_neighborhood)[c(1:6)]) #row_side_colors = c(rep('1',3), rep('2', 3), rep('3',3), rep('4',3), rep('5',3), rep('6',3)
...
# For Neurons - specific analysis
  ...
  #calculate heatmap
    ...
    heat_gr_HiAD <- t(t(heat_gated_regions[c(1:5),other_struct]) / DG_vector[,1])
    heat_gr_MedAD <- t(t(heat_gated_regions[c(6:10),other_struct]) / DG_vector[,2])
    heat_gr_Ctrl <- t(t(heat_gated_regions[c(11:15),other_struct]) / DG_vector[,3])
    
    #divergent color palette -> scale_fill_gradient_fun
    ...
    #full heatmap
    heatmaply(heat_gated_regions[c(1:15),panel[-c(1:4,44:45)]], scale='column', grid_gap = 1, Rowv = TRUE, Colv = TRUE, labRow = rownames(heat_gated_regions)[c(1:15)], row_side_colors = c(rep('HiAD',5), rep('MedAD', 5), rep('Ctrl',5)))
    
    #by sample heatmap
    h1 <- heatmaply(heat_gr_HiAD, scale = 'column', Rowv = FALSE, Colv = TRUE, labRow = rownames(heat_gated_regions)[c(1:5)]) # Ctrl
    h2 <- heatmaply(heat_gr_MedAD, scale = 'column', Rowv = FALSE, Colv = TRUE, labRow = rownames(heat_gated_regions)[c(6:10)]) # MedAD
    h3 <- heatmaply(heat_gr_Ctrl, scale = 'column', Rowv = FALSE, Colv = TRUE, labRow = rownames(heat_gated_regions)[c(11:15)]) # HiAD
    #plot together
  ...
  hi_tau_neurons %>%
    ggplot() +
    aes(x = CD47, y = VGLUT1, fill = run_type_id) +
    geom_point(size = 1L ) +
    geom_density2d(colour = "#FF7F00") +
    scale_colour_gradientn(colours=rainbow(4)) +
    theme_minimal() +
