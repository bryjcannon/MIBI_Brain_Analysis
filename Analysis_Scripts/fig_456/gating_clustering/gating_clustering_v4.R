...
data_indexes = which(with(master_data_all_cuberoot, obj_type_id == "tau-pathies" & obj_size > 4))
master_data_all_cuberoot[data_indexes, "cluster_id"] = 1
data_indexes = which(with(master_data_all_cuberoot, obj_type_id == "amyloid-pathies" & obj_size > 9))
master_data_all_cuberoot[data_indexes, "cluster_id"] = 2
data_indexes = which(with(master_data_all_cuberoot, obj_type_id == "microglia-processes" & obj_size >= 10 & obj_size <= 1000))
...
# gating cells based on biaxial plots (versions of above)
  ...
  # oligodendrocytes
  data_indexes = which(with(master_data_all_cuberoot, obj_type_id == 'cells' & (MAG > 12 | MBP > 13)))
  master_data_all_cuberoot[data_indexes, "cluster_id"] = 5
  
  # endothelial
  data_indexes = which(with(master_data_all_cuberoot, obj_type_id == 'cells' & (MAG <= 12 & MBP <= 13) & ((CD31 > 6 & MAP2 < 15) | (CD105 > 7 & MAP2 < 18) | (MCT1 > 7 | MAP2 < 18)))) 
  master_data_all_cuberoot[data_indexes, "cluster_id"] = 8
  
  # microglia
  data_indexes = which(with(master_data_all_cuberoot, obj_type_id == 'cells' & (MAG <= 12 & MBP <= 13) & ((CD31 <= 6 | MAP2 >= 15) | (CD105 <= 7 | MAP2 >= 18) | (MCT1 <= 7 | MAP2 >= 18)) & ((CD45 > 7 & MAP2 < 15) | (Iba1 > 10))))
  ...
  data_indexes = which(with(master_data_all_cuberoot, obj_type_id == 'cells' & (HistoneH3Lyo <= 5 | obj_size > 800 | MajorAxisLength / MinorAxisLength > 5)))
  master_data_all_cuberoot[data_indexes, "cluster_id"] = 11
  
  # gate out endothelial cells at the edge of tissue (near gold)
  data_indexes = which(with(master_data_all_cuberoot, (cluster_id == 8 | cluster_id == 1 | cluster_id == 2) & run_type_id == 'Ctrl1' & point_id %in% c(22,24,25,27,26,55,56,85,86))) 
  master_data_all_cuberoot[data_indexes, "cluster_id"] = 12
  data_indexes = which(with(master_data_all_cuberoot, (cluster_id == 8 | cluster_id == 1 | cluster_id == 2) & run_type_id == 'Ctrl2' & point_id %in% c(15,16,45,46,75,76,105)))
  master_data_all_cuberoot[data_indexes, "cluster_id"] = 12
  data_indexes = which(with(master_data_all_cuberoot, (cluster_id == 8 | cluster_id == 1 | cluster_id == 2) & run_type_id == 'Ctrl3' & point_id %in% c(1,50)))
  master_data_all_cuberoot[data_indexes, "cluster_id"] = 12
  data_indexes = which(with(master_data_all_cuberoot, (cluster_id == 8 | cluster_id == 1 | cluster_id == 2) & run_type_id == 'HiAD' & point_id %in% c(1,28,29,168,169,196,195)))
  master_data_all_cuberoot[data_indexes, "cluster_id"] = 12
  data_indexes = which(with(master_data_all_cuberoot, (cluster_id == 8 | cluster_id == 1 | cluster_id == 2) & run_type_id == 'MedAD' & point_id %in% c(156-1,157-1,182-1,183-1,184-1,208-1,207-1,209-1,210-1,211-1,234-1,233-1,232-1,235-1,236-1,237-1,260-1,259-1,258-1,257-1,1-1,2-1,26-1,27-1,25-1)))
  master_data_all_cuberoot[data_indexes, "cluster_id"] = 12
  
  # gate out unwanted border regions in MedAD
  data_indexes = which(with(master_data_all_cuberoot, run_type_id == 'MedAD' & point_id %in% c(156-1,157-1,182-1,183-1,184-1,208-1,207-1,209-1,210-1,211-1,234-1,233-1,232-1,239-1,235-1,236-1,237-1,238-1,239-1,240-1,241-1,260-1,259-1,258-1,257-1,256-1,255-1,1-1,2-1,26-1,27-1,25-1,254-1,253-1)))
  master_data_all_cuberoot[data_indexes, "cluster_id"] = 13
  
  # set ungated objects and cells to non-zero cluster_id
  data_indexes = which(with(master_data_all_cuberoot, cluster_id == 0))
  master_data_all_cuberoot[data_indexes, "cluster_id"] = 14
...
# output df to csvs for matlab use - CytoMAP
  ...
  subr <- levels(master_data_all_cuberoot_c$Subregions) 
  ...
  cluster_names <- c('tau-pathies', 'amyloid-pathies', 'microglia-processes', 'vessels', 'oligodendrocytes', 'astrocytes', 'microglia', 'endothelial', 'neurons', 'non_immune_neuroglia')
  
  by_subregion <- FALSE
  
  for (level in 1:length(run_levels)) {
    path_name <- paste0("/Volumes/BryJC_Stanford/paper1_analysis/Fig6/single_object_pipeline/cytomap_files/", run_levels[level], "_mp")
    dir.create(path_name, showWarnings = FALSE)
    for (cluster_num in c(1,2,3,7:10)) {
      if (by_subregion == TRUE) {
        for (sr in 1:14) {
          data_to_write <- master_data_all_cuberoot_cc %>% filter(run_type_id == level & cluster_id == cluster_num & Subregions == sr) %>% na.omit()
          if (!empty(data_to_write)) {
            write_csv(data_to_write, paste0(path_name,'/', 'final2subr_', subr[sr], '_', cluster_names[cluster_num], '.csv'))
          }
        }
      }
      else {
        data_to_write <- master_data_all_cuberoot_cc %>% filter(run_type_id == level & cluster_id == cluster_num) %>% na.omit()
        write_csv(data_to_write, paste0(path_name,'/', cluster_names[cluster_num], '-mp.csv'))
      }
    }
  }
  ...
  master_data_all_cuberoot_c <- filter(master_data_all_cuberoot, cluster_id == 1 | cluster_id == 2 | cluster_id == 3 | cluster_id == 5 | cluster_id == 6 | cluster_id == 7 | cluster_id == 8 | cluster_id == 9)
  levels(master_data_all_cuberoot_c$run_type_id) <- c('HiAD', 'MedAD', 'Ctrl', 'Ctrl', 'Ctrl')
  run_levels <- levels(master_data_all_cuberoot_c$run_type_id)
