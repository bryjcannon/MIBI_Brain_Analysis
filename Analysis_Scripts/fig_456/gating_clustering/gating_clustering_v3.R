...
# gating cells based on biaxial plots (versions of above)
  ...
  # endothelial
  data_indexes = which(with(master_data_all_cuberoot, obj_type_id == 'cells' & ((CD31 > 6 & MAP2 < 15) | (CD105 > 7 & MAP2 < 18) | (MCT1 > 7 | MAP2 < 18)))) 
  ...
  # microglia
  data_indexes = which(with(master_data_all_cuberoot, obj_type_id == 'cells' & ((CD45 > 7 & MAP2 < 15) | (Iba1 > 10))))
  data_indexes = setdiff(data_indexes, gatedIndexes)
  ...
  data_indexes = which(with(master_data_all_cuberoot, obj_type_id == 'cells' & cluster_id == 5))
