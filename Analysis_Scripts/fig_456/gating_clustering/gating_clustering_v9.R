#### Manual Gating & Clustering ####
  ...
  data_indexes = which(with(gated_data, (obj_type_id == "tau-pathies" | obj_type_id == "amyloid-pathies") & ((Orientation < 5 & Orientation > -5) & Eccentricity > 0.8))) # get rid of abnormaly selected edge objects
...
#### Clustering Methods ####
  ...
  #### Bryan Code ####
    ...
    # run FlowSOM on the above flowFrame which has already had transformation performed 
      # to adjust initial cluster numbers: xdim=10, ydim=10
    ...
    # calculate heatmap from all clusters
    ...
    heatmaply(heat_f[,neuron_struct3])
    heatmaply(heat_f[,clustering_markers])
    
    # biaxial plots for FlowSOM_id clustering
    fsom_clustering_data %>%
      #filter(obj_type_id %in% "cells") %>%
      #filter(Meta %in% c(109)) %>%
      dplyr::group_by(FlowSOM_ids) %>%
      ...
      ggplot() +
      aes(x = VGLUT1, y = VGAT, colour = Meta) +
      geom_point(size = 5L ) +
      geom_density2d(colour = "#FF7F00") +
      ...
      ### reassign metaclusters based upon plot inspection ###
        ...
        fsom_clustering_data[data_indexes, "Meta"] = 109
        # non-immune glia
        ...
        fsom_clustering_data[data_indexes, "Meta"] = 110 
        # microglia
        data_indexes = which(with(fsom_clustering_data, FlowSOM_ids %in% c(100,90,99,89,80,79,70,69,98,88,97,87,96,86,95)))
        fsom_clustering_data[data_indexes, "Meta"] = 107 
        # endothelial
        data_indexes = which(with(fsom_clustering_data, FlowSOM_ids %in% c(19,20,10,9,8,7,91,92,81,82,18)))
        fsom_clustering_data[data_indexes, "Meta"] = 108 
        # unknown
        data_indexes = which(with(fsom_clustering_data, !Meta %in% c(109,110,107,108, 'NA')))
        check <- fsom_clustering_data[data_indexes, "Meta"]
        fsom_clustering_data[data_indexes, "Meta"] = 200
        # gate out Meta of endothelial cells at the edge of tissue (near gold)
        data_indexes = which(with(fully_FlowSOM, (Meta == 108) & run_type_id == 'Ctrl1' & point_id %in% c(22,23,24,25,26,27,55,56,85,86))) 
        fsom_clustering_data[data_indexes, "Meta"] = 200
        data_indexes = which(with(fully_FlowSOM, (Meta == 108) & run_type_id == 'Ctrl2' & point_id %in% c(15,16,45,46,75,76,105)))
        fsom_clustering_data[data_indexes, "Meta"] = 200
        data_indexes = which(with(fully_FlowSOM, (Meta == 108) & run_type_id == 'Ctrl3' & point_id %in% c(1,50)))
        fsom_clustering_data[data_indexes, "Meta"] = 200
        data_indexes = which(with(fully_FlowSOM, (Meta == 108) & run_type_id == 'HiAD' & point_id %in% c(1,28,29,168,169,196,195)))
        fsom_clustering_data[data_indexes, "Meta"] = 200
        data_indexes = which(with(fully_FlowSOM, (Meta == 108) & run_type_id == 'MedAD' & point_id %in% c(156-1,157-1,182-1,183-1,184-1,208-1,207-1,209-1,210-1,211-1,234-1,233-1,232-1,235-1,236-1,237-1,260-1,259-1,258-1,257-1,1-1,2-1,26-1,27-1,25-1)))
        fsom_clustering_data[data_indexes, "Meta"] = 200
      ...
      ### create new data.frame 'fully_FlowSOM' with combined newly labeled cells and ez_objects ###
        ...
        non_flowSOM_data$Meta <- 200
        fully_FlowSOM <- rbind(fsom_clustering_data, non_flowSOM_data)
        
        # fix ez_labeling for Meta (not technically a FlowSOM_id)
        data_indexes = which(with(fully_FlowSOM, cluster_id == 1))
        fully_FlowSOM[data_indexes, "Meta"] = 1 # tau tangles
        data_indexes = which(with(fully_FlowSOM, cluster_id == 2))
        fully_FlowSOM[data_indexes, "Meta"] = 2 # amyloid plaques
        data_indexes = which(with(fully_FlowSOM, cluster_id == 3))
        fully_FlowSOM[data_indexes, "Meta"] = 3 # microglia-processes
        data_indexes = which(with(fully_FlowSOM, cluster_id == 4))
        fully_FlowSOM[data_indexes, "Meta"] = 4 # vessels
        
        # gate out Meta of endothelial cells at the edge of tissue (near gold)
        data_indexes = which(with(fully_FlowSOM, (Meta == 108) & run_type_id == 'Ctrl1' & point_id %in% c(22,23,24,25,26,27,55,56,85,86))) 
        fully_FLowSOM[data_indexes, "Meta"] = 200
        data_indexes = which(with(fully_FlowSOM, (Meta == 108) & run_type_id == 'Ctrl2' & point_id %in% c(15,16,45,46,75,76,105)))
        fully_FlowSOM[data_indexes, "Meta"] = 200
        data_indexes = which(with(fully_FlowSOM, (Meta == 108) & run_type_id == 'Ctrl3' & point_id %in% c(1,50)))
        fully_FlowSOM[data_indexes, "Meta"] = 200
        ...
        for (cell in seq_along(fully_FlowSOM)) {
          if (fully_FlowSOM[cell,]$Meta == 107) {
            ...
            data_indexes = which(with(fully_FlowSOM, (Meta == 3 & (x_centroid < abs(x-MajorAxisLength/2) | x_centroid < abs(x+MajorAxisLength/2) & (y_centroid < abs(x-MajorAxisLength/2) | y_centroid < abs(x+MajorAxisLength/2))))))
            fully_FlowSOM[data_indexes, "Meta"] = 200
          }
        }
        
        data_indexes = which(with(fully_FlowSOM, obj_type_id == 1))
        fully_FlowSOM[data_indexes, "obj_type_id"] = 'cells' # reconvert 1 back to cells from earlier as.numeric conversion
        
        filter(fully_FlowSOM, obj_type_id == 'cells') %>% select(Meta) %>% table() # check Meta
  ...
  #### Testing with umap ####
    ...
    #umap testing
      ...
      custom_config$b = 1 # higher numbers brings different clusters closer
      ...
      p1_U <- filter(obj_umap_plot_all_data, UMAP1 > -20 & UMAP2 > -20) %>%
        ggplot(aes(x = UMAP1, y = UMAP2, color=FlowSOM_ids)) +
        geom_point(size = 0.5) + 
...
#### Write Files for Spatial Analysis Pipelines ####
  ...
  for (level in 1:length(run_levels)) {
    path_name <- paste0("/Volumes/BryJC_Stanford/paper1_analysis/Fig6/single_object_pipeline/cytomap_files/", run_levels[level], "_FSOM3_meta")
    dir.create(path_name, showWarnings = FALSE)
    for (cluster_num in c(1:4,107:110)) {
      if (by_subregion == TRUE) {
        for (sr in 1:14) {
          data_to_write <- cytomap_data_num %>% filter(run_type_id == level & Meta == cluster_num & Subregions == sr) %>% na.omit()
        ...
        data_to_write <- cytomap_data_num %>% filter(run_type_id == level & Meta == cluster_num) %>% na.omit()
        if (cluster_num > 100) {cluster_num <- cluster_num-100}
