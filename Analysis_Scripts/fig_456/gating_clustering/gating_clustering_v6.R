  ...
  # gating cells based on biaxial plots (versions of above)
    ...
    for (cell in seq_along(gated_data)) {
      if (gated_data[cell,]$cluster_id == 8) {
        ...
        #filter projections by -10 & + 10 in x and y direction
...
#### Clustering Methods ####
  ...
  #### Bryan Code ####
    ...
    cluster_comparisons <- inner_join(fsom_clustering_data, master_data_all_cuberoot_orig_cid, by = "spatial_id", copy = TRUE) #used to compare manual clustering on un-normalized data
    ...
    metaClustering <- metaClustering_consensus(fSOM2$FlowSOM$map$codes,k=5)
    ...
    fSOM2_clustering$Meta <- as.numeric(fSOM2$metaclustering[fSOM2_clustering[,1]])
      ...
      ### reassign metaclusters based upon plot inspection ###
        # neurons
        data_indexes = which(with(fsom_clustering_data, FlowSOM_ids %in% c(11,1,2,12,22,32,31,21,71,72,62,61,51,52,42,41))) 
        fsom_clustering_data[data_indexes, "FlowSOM_ids"] = 109
        # non-immune glia
        data_indexes = which(with(fsom_clustering_data, FlowSOM_ids %in% c(28,27,37,38,48,47,46,56,57,36,55,45,35,44,54,43,53)))
        fsom_clustering_data[data_indexes, "FlowSOM_ids"] = 110 
        # microglia
        data_indexes = which(with(fsom_clustering_data, FlowSOM_ids %in% c(100,90,99,89,80,79,70,69,98,88,97,87)))
        fsom_clustering_data[data_indexes, "FlowSOM_ids"] = 107 
        # endothelial
        data_indexes = which(with(fsom_clustering_data, FlowSOM_ids %in% c(19,20,10,9,8,7,18)))
      ...
      ### create new data.frame 'fully_FlowSOM' with combined newly labeled cells and ez_objects ###
        ...
        # gate out endothelial cells, objects at the edge of tissue (near gold)
        data_indexes = which(with(fully_FlowSOM, (FlowSOM_ids == 108 | cluster_id == 1 | cluster_id == 2 | cluster_id == 3 | cluster_id == 4) & run_type_id == 'Ctrl1' & point_id %in% c(22,23,24,25,26,27,55,56,85,86))) 
        fully_FlowSOM[data_indexes, "FlowSOM_ids"] = 202
        data_indexes = which(with(fully_FlowSOM, (FlowSOM_ids == 108 | cluster_id == 1 | cluster_id == 2 | cluster_id == 3 | cluster_id == 4) & run_type_id == 'Ctrl2' & point_id %in% c(15,16,45,46,75,76,105)))
        fully_FlowSOM[data_indexes, "FlowSOM_ids"] = 202
        data_indexes = which(with(fully_FlowSOM, (FlowSOM_ids == 108 | cluster_id == 1 | cluster_id == 2 | cluster_id == 3 | cluster_id == 4) & run_type_id == 'Ctrl3' & point_id %in% c(1,50)))
        fully_FlowSOM[data_indexes, "FlowSOM_ids"] = 202
        data_indexes = which(with(fully_FlowSOM, (FlowSOM_ids == 108 | cluster_id == 1 | cluster_id == 2 | cluster_id == 3 | cluster_id == 4) & run_type_id == 'HiAD' & point_id %in% c(1,28,29,168,169,196,195)))
        fully_FlowSOM[data_indexes, "FlowSOM_ids"] = 202
        data_indexes = which(with(fully_FlowSOM, (FlowSOM_ids == 108 | cluster_id == 1 | cluster_id == 2 | cluster_id == 3 | cluster_id == 4) & run_type_id == 'MedAD' & point_id %in% c(156-1,157-1,182-1,183-1,184-1,208-1,207-1,209-1,210-1,211-1,234-1,233-1,232-1,235-1,236-1,237-1,260-1,259-1,258-1,257-1,1-1,2-1,26-1,27-1,25-1)))
        fully_FlowSOM[data_indexes, "FlowSOM_ids"] = 202
        # gate out edge plaques, tangles
        data_indexes = which(with(fully_FlowSOM, (obj_type_id == "tau-pathies" | obj_type_id == "amyloid-pathies") & ((Orientation < 5 & Orientation > -5) & Eccentricity > 0.8)))
        fully_FlowSOM[data_indexes, "FlowSOM_ids"] = 203
        ...
        for (cell in seq_along(fully_FlowSOM)) {
          if (fully_FlowSOM[cell,]$FlowSOM_ids == 108) {
            x = fully_FlowSOM[cell,]$x_centroid
            y = fully_FlowSOM[cell,]$y_centroid
            #filter projections by -10 & + 10 in x and y direction
            data_indexes = which(with(fully_FlowSOM, (FlowSOM_ids == 4 & (x_centroid < abs(x-MajorAxisLength/2) | x_centroid < abs(x+MajorAxisLength/2) & (y_centroid < abs(x-MajorAxisLength/2) | y_centroid < abs(x+MajorAxisLength/2))))))
            fully_FlowSOM[data_indexes, "FlowSOM_ids"] = 201
          }
        }
        filter(fully_FlowSOM, obj_type_id == 'cells') %>% select(FlowSOM_ids) %>% table()
...
#### Write Files for Spatial Analysis Pipelines ####
  ...
  levels(cytomap_data$obj_type_id) <- c('cells', 'amyloid-pathies', 'tau-pathies', 'microglia-processes', 'vessels')
  ...
  for (level in 1:length(run_levels)) {
    path_name <- paste0("/Volumes/BryJC_Stanford/paper1_analysis/Fig6/single_object_pipeline/cytomap_files/", run_levels[level], "_FSOM2_parsed")
    ...
    for (cluster_num in c(1:4,107:110)) {
      if (by_subregion == TRUE) {
        ...
        if (cluster_num > 100) {cluster_num <- cluster_num-100}
        readr::write_csv(data_to_write, paste0(path_name,'/', cluster_names[cluster_num], '_FSOM2_parsed.csv'))
      }
    }
  }
