...
master_data_ion_cuberoot$cluster_id = 0
# add gate ids for ez_segmented objects
data_indexes = which(with(master_data_ion_cuberoot, obj_type_id == "tau-pathies"))
master_data_ion_cuberoot[data_indexes, "cluster_id"] = 1
data_indexes = which(with(master_data_ion_cuberoot, obj_type_id == "amyloid-pathies"))
master_data_ion_cuberoot[data_indexes, "cluster_id"] = 2
data_indexes = which(with(master_data_ion_cuberoot, obj_type_id == "microglia-processes"))
master_data_ion_cuberoot[data_indexes, "cluster_id"] = 3
data_indexes = which(with(master_data_ion_cuberoot, obj_type_id == "vessels"))
master_data_ion_cuberoot[data_indexes, "cluster_id"] = 4
...
# gating cells based on biaxial plots (versions of above)
  ...
  # oligodendrocytes
  gate <- master_data_ion_cuberoot %>% filter(obj_type_id == 'cells') %>% filter(MAG > 10 | MBP > 13)
  data_indexes = which(with(master_data_ion_cuberoot, obj_type_id == 'cells' & (MAG > 10 | MBP > 13)))
  #data_indexes = which(with(master_data_ion_cuberoot, gate)) #from dmitry
  #data_indexes = setdiff(data_indexes, gatedIndexes) #from dmitry
  master_data_ion_cuberoot[data_indexes, "cluster_id"] = 5
  master_data_ion_cuberoot[data_indexes, "obj_type_id"] = "oligodendrocytes" # currently doesn't work due to obj_type_id as a factor variable
  #gatedIndexes = c(gatedIndexes, data_indexes) #from dmitry
  
  # astrocytes
  gate <- master_data_ion_cuberoot %>% filter(obj_type_id == 'cells') %>% filter(MAG <= 10 & MBP <= 13) %>% filter((CD31 <= 6 | MAP2 >= 15) | (CD105 <= 7 | MAP2 > 18) | (MCT1 <= 7 | MAP2 > 18)) %>% filter((CD45 <= 7 | MAP2 >= 15) | (Iba1 <= 10)) %>%
    filter(GFAP > 8 & MAP2 < 10) %>% filter(GFAP / MAP2 > 1.2) %>% dim()
  data_indexes = which(with(master_data_ion_cuberoot, obj_type_id == 'cells' & (MAG <= 10 & MBP <= 13) & ((CD31 <= 6 | MAP2 >= 15) | (CD105 <= 7 | MAP2 > 18) | (MCT1 <= 7 | MAP2 > 18)) & ((CD45 <= 7 | MAP2 >= 15) | (Iba1 <= 10)) &
                            (GFAP > 8 & MAP2 < 10)  & (GFAP / MAP2 > 1.2)))
  master_data_ion_cuberoot[data_indexes, "cluster_id"] = 6
  master_data_ion_cuberoot[data_indexes, "obj_type_id"] = "astrocytes"
  
  # microglia
  gate <-  master_data_ion_cuberoot %>% filter(obj_type_id == 'cells') %>% filter(MAG <= 10 & MBP <= 13) %>% filter((CD31 <= 6 | MAP2 >= 15) | (CD105 <= 7 | MAP2 >= 18) | (MCT1 <= 7 | MAP2 >= 18)) %>% filter((CD45 > 7 & MAP2 < 15) | (Iba1 > 10))
  data_indexes = which(with(master_data_ion_cuberoot, obj_type_id == 'cells' & (MAG <= 10 & MBP <= 13) & ((CD31 <= 6 | MAP2 >= 15) | (CD105 <= 7 | MAP2 >= 18) | (MCT1 <= 7 | MAP2 >= 18)) & ((CD45 > 7 & MAP2 < 15) | (Iba1 > 10))))
  master_data_ion_cuberoot[data_indexes, "cluster_id"] = 7
  master_data_ion_cuberoot[data_indexes, "obj_type_id"] = "microglia"
  
  # endothelial
  gate <- master_data_ion_cuberoot %>% filter(obj_type_id == 'cells') %>% filter(MAG <= 10 & MBP <= 13) %>% filter((CD31 > 6 & MAP2 < 15) | (CD105 > 7 & MAP2 < 18) | (MCT1 > 7 | MAP2 < 18))
  data_indexes = which(with(master_data_ion_cuberoot, obj_type_id == 'cells' & (MAG <= 10 & MBP <= 13) & ((CD31 > 6 & MAP2 < 15) | (CD105 > 7 & MAP2 < 18) | (MCT1 > 7 | MAP2 < 18)))) 
  master_data_ion_cuberoot[data_indexes, "cluster_id"] = 8
  master_data_ion_cuberoot[data_indexes, "obj_type_id"] = "endothelial"
  ...
  data_indexes = which(with(master_data_ion_cuberoot, obj_type_id == 'cells' & (MAG <= 10 & MBP <= 13) & ((CD31 <= 6 | MAP2 >= 15) | (CD105 <= 7 | MAP2 > 18) | (MCT1 <= 7 | MAP2 > 18)) & ((CD45 <= 7 | MAP2 >= 15) | (Iba1 <= 10)) &
                              (GFAP <= 8 | MAP2 >= 10) & (GFAP / MAP2 <= 1.2))) 
  master_data_ion_cuberoot[data_indexes, "cluster_id"] = 9
  master_data_ion_cuberoot[data_indexes, "obj_type_id"] = "neurons"

  # gat out anything with low histone
  data_indexes = which(with(master_data_ion_cuberoot, obj_type_id == 'cells' & (HistoneH3Lyo <= 5 | obj_size > 1000 | MajorAxisLength / MinorAxisLength > 10)))
  master_data_ion_cuberoot[data_indexes, "cluster_id"] = 10
  master_data_ion_cuberoot[data_indexes, "obj_type_id"] = "lowH3"
  
  # check cluster_ids
  filter(master_data_ion_cuberoot, obj_type_id == 'cells') %>% select(cluster_id) %>% table()
  select(master_data_ion_cuberoot, cluster_id) %>% table()
...
# output df to csvs for matlab use - CytoMAP
  master_data_ion_cuberoot_c <- master_data_ion_cuberoot
  #converting levels to num for csv
  levels(master_data_ion_cuberoot_c$obj_type_id) <- c('cells', 'amyloid-pathies', 'tau-pathies', 'microglia-processes', 'vessels')
  
  levels(master_data_ion_cuberoot_c$Subregions) <- c("Alveus", "Au", "CA1", "CA2", "CA3", "CA4", "DG", "EC", "Fimbria", "ML", "PreSubiculum", "SRL", "Subiculum", "TH-LV")
  
  levels(master_data_ion_cuberoot_c$run_type_id) <- c('HiAD_modded', 'MedAD_modded', 'Ctrl_modded', 'Ctrl_modded', 'Ctrl_modded')
  run_levels <- levels(master_data_ion_cuberoot_c$run_type_id)
  
  master_data_ion_cuberoot_cc <- mutate_if(master_data_ion_cuberoot_c, is.factor, ~ as.numeric(.x))
  ...
  for (level in 1:length(run_levels)) {
    ...
    for (cluster_num in 1:length(cluster_names)) {
      data_to_write <- master_data_ion_cuberoot_cc %>% filter(run_type_id == level & cluster_id == cluster_num) %>% na.omit()
...
# output df to txts for python use - SVCA
  ...
  master_data_ion_cuberoot_c <- filter(master_data_ion_cuberoot, cluster_id == 1 | cluster_id == 2 | cluster_id == 5 | cluster_id == 6 | cluster_id == 7 | cluster_id == 8 | cluster_id == 9)
  levels(master_data_ion_cuberoot_c$run_type_id) <- c('HiAD', 'MedAD', 'Ctrl', 'Ctrl', 'Ctrl')
  run_levels <- levels(master_data_ion_cuberoot_c$run_type_id)
  ...
  for (level in run_levels) {
    ...
    run_subregions <- levels(filter(master_data_ion_cuberoot_c, run_type_id == 'HiAD')$Subregions)
    for (subr in run_subregions) {
      ...
      data_to_write <- master_data_ion_cuberoot_c %>% filter(run_type_id == level)
