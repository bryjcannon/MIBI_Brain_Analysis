...
% output folder
output_folder = "/Volumes/BryJC_Stanford/other/ForDmitry/fig6_carpet_voronoi_plaques";
...
for run = 1:length(run_types)
    ...
    elseif method == "voronoi"
        ...
        for i = 1:sc_size(1)
            % if subset is active
            if subset == 1 && ~strcmp(start_coord.(genvarname(subset_name))(i), subset_term)
                continue
            end
            if mod(i, 100) == 0
                disp(join(['Processing polygon # ', string(i)]))
            end
            A = V(R{i},:);
            B = A(any(~isinf(A),2),:); % omit points at infinity
            bw = poly2mask(B(:,1), B(:,2), sm_size(1), sm_size(2));
            new_map(bw == 1) = color_map(start_coord.(genvarname(color_name))(i));
        end
