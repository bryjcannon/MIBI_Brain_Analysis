...
#### Import All ####
  ...
  pixel_anat_info <- read_csv('/Volumes/BryJC_Stanford/Analysis/KV_BJC_MIBI_Brain_Paper_1/Fig_MedRes/data_outputs/mask_expansion_pixel_capture/pixel_data_anat/anat_only/Anat_pixel_info.csv',  col_names = TRUE)
  pixel_anat_info$Run <- factor(pixel_anat_info$Run, levels = c(1,2,3)) # factorize run
  levels(pixel_anat_info$Run) <- c('HiAD', 'MedAD', 'Ctrl')
  pixel_anat_info$Mask <- factor(pixel_anat_info$Mask) # factorize region

#### Import Tangles & Plaques ####
  # Read in csv with pixel per mask, run info (generated in MATLAB)
  pixel_anat_info_v2 <- c();
  for (i in c('plaques')) {
    ...
    pixel_anat_info_v2 <- rbind(pixel_anat_info_v2, temp)
    rm(temp)
  }

#### Create an alternative data.frame where all pixels with no + value are excluded ####
  pixel_anat_info_pos <- filter_all(pixel_anat_info, any_vars(. > 0))
...
#### Marker Summary Analysis Code ####
  ...
  # violin plot
  gathered_pixel_data <- gather(pixel_anat_info_pos, key = "markers", value = "expr", -c("Run", "Mask"))
  ggplot(pixel_anat_info_pos) +
  ...
  for (i in c('Ctrl','MedAD','HiAD')) {
         filtered_data_corr <- filter(pixel_anat_info, Run == i, Mask == 4)
         m <- cor(filtered_data_corr[3:ncol(pixel_anat_info)])
...
#### Synaptic Density Analysis Code ####
  ...
  ex_synapses <- filter(pixel_anat_info, PSD95 > 0 & (Synaptophysin > 0 | VGLUT1 > 0 | VGLUT2 > 0))
  ex_synapses_t <- filter(pixel_anat_info, PHF1Tau > 0 & (PSD95 > 0 & (Synaptophysin > 0 | VGLUT1 > 0 | VGLUT2 > 0)))
  ex_synapses_a <- filter(pixel_anat_info, (Amyloidbeta140 > 0 | Amyloidbeta142 > 0 | PanAmyloidbeta1724 > 0) & (PSD95 > 0 & (Synaptophysin > 0 | VGLUT1 > 0 | VGLUT2 > 0)))
  in_synapses <- filter(pixel_anat_info, PanGAD6567 > 0 & VGAT > 0)
  in_synapses_t <- filter(pixel_anat_info, PHF1Tau > 0 & (PanGAD6567 > 0 & VGAT > 0))
  in_synapses_a <- filter(pixel_anat_info, (Amyloidbeta140 > 0 | Amyloidbeta142 > 0 | PanAmyloidbeta1724 > 0) & (PanGAD6567 > 0 & VGAT > 0))
  
  ex_syn_percent <- table(ex_synapses$Run) / table(pixel_anat_info$Run) * 100
  in_syn_percent <- table(in_synapses$Run) / table(pixel_anat_info$Run) * 100
...
#### Marker density testing ####
  # for binarized format
    pixel_anat_info_bin <- pixel_anat_info
    pixel_anat_info_bin[3:ncol(pixel_anat_info_bin)] <- ifelse(pixel_anat_info_bin[3:ncol(pixel_anat_info_bin)] > 0, 1, 0)
  # collect pixel groups
  pixel_anat_info_bin %>% 
    dplyr::group_by(Mask, Run) %>% 
    dplyr::summarize_if(is.numeric, funs(sum)) %>% # mean or sum
    ungroup() -> pixel_anat_norm
  pixel_anat_norm_area <- apply(pixel_anat_norm[,3:ncol(pixel_anat_norm)], 2, function(x) x/pixel_anat_area[3])
  pixel_anat_norm_area <- unlist(pixel_anat_norm_area, recursive = FALSE)
  names(pixel_anat_norm_area) <- sub(".area", "", names(pixel_anat_norm_area))
  pixel_anat_norm_area <- cbind(pixel_anat_norm[1:2], pixel_anat_norm_area)
  
  # total counts
  AP <- ggplot(pixel_anat_norm_area) +
    aes(x = Mask, fill = Run, weight = MFN2) +
    geom_bar(position = position_dodge2(width = 0.9, preserve = "single")) +
    scale_fill_manual(values = color4) +
    theme_minimal(25) +
    labs(title = "Total positive pixel counts: MFN2", subtitle = "area normalized (mm2)", y = "total pixels") +
    facet_grid(cols = vars(Run))
  AP
  ggsave(filename = 'total_positive_pixel_counts_MFN2.pdf', plot = AP, width = 15, height = 10, dpi = 300, device = 'pdf',
