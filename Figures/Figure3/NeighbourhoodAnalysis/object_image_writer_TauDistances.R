...
##### FILE & DATA SETTINGS - user must adjust these! #####
  ...
  data_folder = '/Volumes/KausaliaHD 1/AllProjects/BRAIN DATA/MIBI/MIBIData Matlab/May_June2019/HiResScans_uci2717J/'
  setwd(data_folder)
  
  # run on data that has been already read and transformed into R using ezAnalysis script (script will output all_data)
  library(readr)
  data_for_images <- read_csv("EzSegData_uci2712J/ezSegDiseaseGrades/ClusteringLineageTangles.csv") 
  data_for_images$FlowSOM_ids <- as.numeric(as.factor(data_for_images$ClusteringLineage))
  data_for_images$run_type_id <- 'HiAD'
  data_for_images$obj_type_id <- 'tauTangles'
  data_for_images$point_id <- data_for_images$Point_num
  data_for_images$obj_id <- data_for_images$Object_num
  
  # name to give this specific image creation run when saving
  save_name <- 'tau_overlays_CL'
  # shorthand for regions or runs, used in run_type_id
  run_names <- c('HiAD')
  # rgb or grayscale
  grayscale <- F
  # object types you plan to cluster
  object_types <- c('tauTangles')
  # boolean denoting if clustering was performed on the objects (T) or not (F)
  objects_clustered <- T
  clustering_variable <- 'FlowSOM_ids'
  include_clusters <- c(1:10) #represents c('tangles','plaques','microglia','endothelial','neurons','non-immune glia') OR de_novo_regions 1-5
  ...
  #To check colors use -> 
  if (grayscale == F) {pie(rep(1, 10), col = palette)}
  ...
  for (i in 1:length(run_names)) {
    ...
    for (p in point_list[[i]]) {
      ...
      for (obj_index in 1:length(object_types)) { 
        ...
        if (grayscale == F) {
          obj_mask_r <- matrix(255,resolution_dim[1], resolution_dim[2])
          obj_mask_g <- matrix(255,resolution_dim[1], resolution_dim[2])
          obj_mask_b <-matrix(255,resolution_dim[1], resolution_dim[2])
        }
        ...
        for (uniq_object in unique(pointdata$obj_id)){
          ...
          if (objects_clustered == T & grayscale == F) {
            obj_mask_r[obj_mask == uniq_object] = col2rgb(palette)[1, as.numeric(subset(pointdata, obj_id == uniq_object)[clustering_variable])]
            obj_mask_g[obj_mask == uniq_object] = col2rgb(palette)[2, as.numeric(subset(pointdata, obj_id == uniq_object)[clustering_variable])]
            obj_mask_b[obj_mask == uniq_object] = col2rgb(palette)[3, as.numeric(subset(pointdata, obj_id == uniq_object)[clustering_variable])]
          }
        ...
        dir.create(paste0(data_folder, "/PHF1Tau_CLOverlay_Images"), showWarnings = FALSE)
        dir.create(paste0(data_folder, "/PHF1Tau_CLOverlay_Images", "/", save_name), showWarnings = FALSE)
        dir.create(paste0(data_folder, "/PHF1Tau_CLOverlay_Images", "/", save_name, "/", run_names[i]), showWarnings = FALSE)
        dir.create(paste0(data_folder, "/PHF1Tau_CLOverlay_Images", "/", save_name, "/", run_names[i], "/", type_of_image), showWarnings = FALSE)
        dir.create(paste0(data_folder, "/PHF1Tau_CLOverlay_Images", "/", save_name, "/", run_names[i], "/", type_of_image, "/Point", p), showWarnings = FALSE)
        dir.create(paste0(data_folder, "/PHF1Tau_CLOverlay_Images", "/", save_name, "/", run_names[i], "/", type_of_image, "/Point", p, "/TIFs/"), showWarnings = FALSE)
        writeImage(img, paste0(data_folder, "/PHF1Tau_CLOverlay_Images", "/", save_name, "/", run_names[i], "/", type_of_image, "/Point", p, "/TIFs/", object_types[obj_index],".tif"))
      }
    }
