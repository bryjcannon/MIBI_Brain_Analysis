...
##### FILE & DATA SETTINGS - user must adjust these! #####
  ...
  library(readr)
  data_for_images <- read_csv("EzSegData_uci2712J/ezSegDiseaseGrades/TotalTauvsPHF1TauGrades.csv") 
  data_for_images$FlowSOM_ids <- as.numeric(as.factor(data_for_images$file))
  ...
  include_clusters <- c(1, 2, 3) #represents c('tangles','plaques','microglia','endothelial','neurons','non-immune glia') OR de_novo_regions 1-5
  ...
  palette <- c("#a80077", "#f4dd66", "#67872f") #anno_color = list(amyloidGrade or tauGrade hi = "#a80077", med = "#f4dd66", lo ="#67872f")
  #To check colors use -> 
  if (grayscale == F) {pie(rep(1, 3), col = palette)}
  ...
  for (i in 1:length(run_names)) {
    ...
    for (p in point_list[[i]]) {
      ...
      for (obj_index in 1:length(object_types)) { 
        ...
        dir.create(paste0(data_folder, "/PHF1Tau_himedlo_Images"), showWarnings = FALSE)
        dir.create(paste0(data_folder, "/PHF1Tau_himedlo_Images", "/", save_name), showWarnings = FALSE)
        dir.create(paste0(data_folder, "/PHF1Tau_himedlo_Images", "/", save_name, "/", run_names[i]), showWarnings = FALSE)
        dir.create(paste0(data_folder, "/PHF1Tau_himedlo_Images", "/", save_name, "/", run_names[i], "/", type_of_image), showWarnings = FALSE)
        dir.create(paste0(data_folder, "/PHF1Tau_himedlo_Images", "/", save_name, "/", run_names[i], "/", type_of_image, "/Point", p), showWarnings = FALSE)
        dir.create(paste0(data_folder, "/PHF1Tau_himedlo_Images", "/", save_name, "/", run_names[i], "/", type_of_image, "/Point", p, "/TIFs/"), showWarnings = FALSE)
        writeImage(img, paste0(data_folder, "/PHF1Tau_himedlo_Images", "/", save_name, "/", run_names[i], "/", type_of_image, "/Point", p, "/TIFs/", object_types[obj_index],".tif"))
      }
    }
