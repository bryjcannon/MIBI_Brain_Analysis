...
##### FILE & DATA SETTINGS - user must adjust these! #####
  ...
  palette <- c("#a80077", "#e1e73d", "#56ab2f") #anno_color = list(amyloidGrade or tauGrade c(hi = "#a80077", lo ="#56ab2f", med = "#e1e73d"))
  #To check colors use -> 
  ...
  for (i in 1:length(run_names)) {
    ...
    for (p in point_list[[i]]) {
      ...
      for (obj_index in 1:length(object_types)) { 
        ...
        dir.create(paste0(data_folder, "/Abeta_himedlo_Images"), showWarnings = FALSE)
        dir.create(paste0(data_folder, "/Abeta_himedlo_Images", "/", save_name), showWarnings = FALSE)
        dir.create(paste0(data_folder, "/Abeta_himedlo_Images", "/", save_name, "/", run_names[i]), showWarnings = FALSE)
        dir.create(paste0(data_folder, "/Abeta_himedlo_Images", "/", save_name, "/", run_names[i], "/", type_of_image), showWarnings = FALSE)
        dir.create(paste0(data_folder, "/Abeta_himedlo_Images", "/", save_name, "/", run_names[i], "/", type_of_image, "/Point", p), showWarnings = FALSE)
        dir.create(paste0(data_folder, "/Abeta_himedlo_Images", "/", save_name, "/", run_names[i], "/", type_of_image, "/Point", p, "/TIFs/"), showWarnings = FALSE)
        writeImage(img, paste0(data_folder, "/Abeta_himedlo_Images", "/", save_name, "/", run_names[i], "/", type_of_image, "/Point", p, "/TIFs/", object_types[obj_index],".tif"))
      }
    }
