...
outputFolder = paste0(dataPath,"DiseaseNeighbourhoodEZDeepcell/RealCountsObjects/AmyloidNeighbourhood_100pixeldistance(New)/")
...
nrun = 10
fulldata = read_csv(paste0(dataPath,"RandomRuns/Disease_MatrixByIndividual_Objects/amyloidPlaques_Plaques_counts_100px_1.csv")) # _freqs_ or _counts_  
fulldata = as.matrix(fulldata)
for (runnum in 2:nrun){
  ## loads Amyloid Neighbourhoods
  plot_data = read_csv(paste0(dataPath,"RandomRuns/Disease_MatrixByIndividual_Objects/amyloidPlaques_Plaques_counts_100px_",runnum,".csv"))  
...
p = ggplot(plot_data,aes(x = MantisPopulation, y = Value)) +  # y =  MaxDist, MeanDist, MedianDist
  geom_violin(aes(fill = MantisPopulation), scale = "width", width = 0.6) +
...
ggsave(paste0(outputFolder, "/Violin_AmyloidPlaques_100Pixeldistance.pdf"), plot = p, width = 7, height = 5)
...
p = ggplot(plot_data,aes(x = MantisPopulation, y = Value))+  # y =  MaxDist, MeanDist, MedianDist
  ...
  xlab("") + ylab("") +
  ggtitle(paste0("")) +
  ylim(c(0,300)) + # watch for warnings to see how many outliers were removed in each plot.
  theme_classic(15) +
  theme(plot.title = ggplot2::element_text(face = "bold", size = 10),
        axis.title.x = ggplot2::element_text(face = "bold", size = 10),
        axis.title.y = ggplot2::element_text(face = "bold", size = 10),
        axis.text.x = ggplot2::element_text(angle = 270, hjust = 0, vjust = 0.5))
ggsave(paste0(outputFolder, "/Scatter_AmyloidPlaques_100Pixeldistance.pdf"), plot = p, width = 7, height = 5)

#Boxplot with T-test
myComparison = list(c("astrocyte_process","microglia_process"), c("astrocyte_process","newLmod"), c("newLmod","microglia_process"), c("newLmod","vessel_CD31_CD105"), c("astrocyte_process","vessel_CD31_CD105"), c("microglia_process","vessel_CD31_CD105"))
...
ggsave(paste0(outputFolder, "/Boxplot_AmyloidPlaques_100pxDist.pdf"), plot = p, width = 7, height = 5)
...
pdf(paste0(outputFolder,"/Heatmap1_AmyloidPlaques_100Pixeldistance.pdf"), width = 20, height = 5, onefile = FALSE)
p=pheatmap (plot_data, cluster_rows = F, cluster_cols = T, clustering_callback = callback, border_color = FALSE, scale = "none", main = "Number of \nRandomised microglia/astrocyte/vessel objects around amyloid plaque objects \nPixelDistance = 100",
            color = viridis(100, alpha=1, begin=0, end=1, direction=1, option = "D"), cellheight = 40)
dev.off()
dev.off()

# this maps Rocket color palette
pdf(paste0(outputFolder,"/Heatmap2_AmyloidPlaques_100Pixeldistance.pdf"), width = 20, height = 5, onefile = FALSE)
